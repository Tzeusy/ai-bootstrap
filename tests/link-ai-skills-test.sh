...
printf '%s\n' 'missing frontmatter' > "$bootstrap_dir/skills/plain/SKILL.md"
if "$linker" "$bootstrap_dir" > "$test_root/frontmatter-failure.log" 2>&1; then
    fail "malformed canonical frontmatter should fail"
fi
grep -Fq -- 'Generated by ai-bootstrap/scripts/link-ai-skills.sh' "$bootstrap_dir/.codex/skills/plain/SKILL.md" || fail "failed refresh removed the working Codex wrapper"
write_skill "$bootstrap_dir/skills/plain" "plain" "A flat skill."
