...
There is no separate `project-shape` coordinator binary or dedicated coordinator agent in this package. In this document, **coordinator** means the lead agent that owns intake, routing, synthesis, review, and installation across the four-pillar shape workflow.
...
| New product idea or change request | Translate idea into requirements | doctrine checks, RFC updates, `openspec/` deltas, beads-ready requirements |
...
## Four-Pillar Backbone

![Four-pillar traceability chain](assets/pillar-traceability.svg)
...
The coordinator works top-down. Doctrine grounds RFCs, RFCs ground specs, and specs ground execution. Topology is cross-cutting: it should be updated anywhere the placement of components, boundaries, or data flow matters.
...
2. Synthesize draft doctrine first, then design contracts, then specs, while starting topology as soon as the architecture track is clear.
...
5. Hand the sharpened output to issue generation or downstream execution workflows.
...
1. Extract only the layman-relevant narrative from each pillar.
...
Use `/excalidraw-diagram` when diagrams materially improve comprehension. If the renderer is unavailable, degrade to Mermaid or prose without weakening the review discipline or the four-pillar ordering.
