...
- Category (infer from naming convention or content)
...
- Read the project's CLAUDE.md, README, or architecture docs to understand the
  directory layout, conventions, and key source paths
- Walk each major source directory and identify modules, services, API routes,
  configuration, tools, and tests
- Capture for each implementation unit: name, what it does (read docstrings/comments),
  which tools/endpoints it exposes
...
- Group by priority: core infrastructure gaps > domain logic gaps > UI/cosmetic gaps
...
2. For each gap, invoke `/beads-writer` to create a well-structured bead:
   ...
   - Priority: P1 for core/infrastructure, P2 for domain logic, P3 for UI/cosmetic
...
database schemas unless they are load-bearing contracts (e.g., shared APIs or schemas
that other components depend on).

### Use Subagents Aggressively for Investigation
Always dispatch subagents (Agent tool with `subagent_type=Explore` or
`subagent_type=general-purpose`) for investigation work rather than trying to read
everything in the main thread. Typical patterns:
- One subagent per spec category to read and summarize requirements
- One subagent per major source directory to inventory features and modules
...
### Spec Naming
Adopt a `{category}-{name}` convention. Common categories:
...
- One agent per spec category
- One agent per major source directory for implementation inventory
