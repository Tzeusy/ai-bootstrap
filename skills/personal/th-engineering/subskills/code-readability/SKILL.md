...
Code is read far more than written. Judges whether a future reader — human or
agent, without the author present — can reconstruct intent, predict behavior,
...
1. **Names state intent, at the right altitude** — Name says what the thing
...
2. **Functions do one thing at one altitude** — Mix abstraction levels
   (business policy next to byte-twiddling) only with strong justification.
   Extract when a block needs a comment to say what it does; inline when an
   indirection exists only to be called once and hides the flow.
3. **Control flow is followable top-to-bottom** — Early returns over nested
   conditionals. No action-at-a-distance: reader should not need to know about
   a decorator, hook, or global to predict what a call does.
...
5. **Comments state constraints, not narration** — Comment says what the code
   cannot: the invariant, the external quirk, the why-not-the-obvious-way.
   Comments that restate the next line, or talk to a reviewer about the change
   ("updated to fix…"), get deleted.
6. **Consistency beats local preference** — Match the surrounding file's
   ...
   in a vacuum. A diff that switches styles mid-module adds permanent reading
   cost for transient taste.
...
1. Read the change as a stranger: top of the diff to bottom, no author
...
3. Prefer fixes that shrink the diff's concept count over fixes that add
   structure. Deleting an abstraction usually beats documenting it.
