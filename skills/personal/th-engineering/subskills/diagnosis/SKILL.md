...
A diagnosis claims cause; an unverified causal claim ships the wrong fix. This
subskill grades the evidence produced while debugging: how the failure is
reproduced, how hypotheses are eliminated, what proof a root-cause story must
carry before a fix is trusted.
...
- A performance regression needs locating, not lamenting
...
repeatable experiment; the fix is proven by the same loop that reproduced the
failure. "I read the code and I'm fairly sure" is a hypothesis, not a
diagnosis.
...
1. **A feedback loop exists before theorizing** — Build the cheapest
   ...
   script. Loop quality is speed, signal sharpness, determinism — improving the
   loop is diagnostic progress, not a detour.
...
3. **Hypotheses are ranked and falsifiable before instrumentation** — Write
   ...
   observation that kills it. Instrument to *discriminate between* hypotheses,
   never to "see what's happening".
...
5. **The fix is proven by the loop** — Red before, green after, same loop. "I
   can no longer reproduce it" without a recorded loop run is not evidence;
   neither is a passing suite that never reproduced the failure.
6. **A regression test lands at the correct seam** — The test exercises the
   layer where the cause is observable, not three mocks away. If no such seam
   exists, say so: that is an architectural finding
...
7. **Performance regressions are measured before blamed** — Establish a
   baseline number, then bisect (commits, configs, or inputs) against it. A
   perf fix claim carries before/after measurements under the same conditions,
   never "feels faster".
...
3. **Discriminate** (bar 4) — add tagged instrumentation separating the top
   hypotheses; run the loop; cross off what the evidence kills.
