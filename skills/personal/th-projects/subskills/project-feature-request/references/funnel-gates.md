...
else glance for the five pillars manually.
**Output**: one line per pillar — present/absent, and whether it constrains
this request.
**Exit**: you know which later gates have a normative source, which are
[Inferred].
...
**Do**: grill one question at a time (see SKILL.md "Interview Discipline" —
carry a recommended answer, explore the codebase before asking the human)
until you can state, in the requester's own language:
...
- **Motif** — the recurring need this instances (a request is one instance;
  the spec names the pattern so the design generalizes the right amount and no
  further)

Challenge patterns (borrow from
...
**Kill**: requester cannot produce a concrete instance — park it as an
observation, not a feature.

## Gate 2 — Doctrine

**Input**: Gate 1 output; `about/heart-and-soul/` if present.
**Do**: check the request against vision, non-negotiables, and explicit scope
...
- **Conflict**: cite the contradicted principle; outcome is *reject* or
  *escalate a doctrine change* (full alignment required), never a quiet
  exception
- **No doctrine**: record your alignment judgment as [Inferred]; flag that the
  project is accumulating direction debt
...
**Do**: name the components touched, boundaries crossed, integration points
added, and where the new behavior is observable.
**Output**: a placement statement — "lives in X, talks to Y over Z, changes
the A↔B contract / changes no contracts".
**Exit**: someone who knows the codebase could point at the directories.
**Kill / loop**: honest answer is "everywhere" → it's a program not a feature.
Return to Gate 1 and split by motif.
...
stating what would have to become true, and park.
...
testable as written.
**Kill / loop**: a requirement resists WHEN/THEN phrasing — still a feeling,
not a behavior. Return to Gate 1 for that slice.

## Gate 6 — Engineering Bar

**Input**: Gate 5 spec delta; `about/craft-and-care/` if present.
**Do**: import applicable standards into acceptance criteria —
- Testing: what proves each scenario (unit/integration/manual), regression
  expectations
- Observability: what gets logged/measured so behavior is verifiable in
  production
