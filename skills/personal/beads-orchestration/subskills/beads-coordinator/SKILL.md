...
description: Coordinate Beads issue execution by continuously selecting the highest-priority ready issue from `bd ready`, claiming it with a durable lease, and dispatching a fresh worker in an isolated beads worktree. Use for unattended parallel issue throughput in Beads-backed repos.
metadata:
  owner: tze
  authors:
    - tze
...
- **Lease before mutation.** Verify lease ownership and renew if near expiry
  before any `bd` mutation. Never overwrite or out-time a live foreign lease.
...
| [`references/runtime-and-safety.md`](references/runtime-and-safety.md) | dispatch or mutate safely | Model-selection tables, durable lease model + stall thresholds, mutation safety, closure rule, runtime dispatch notes, quality gates |
...
2. Create a coordinator session ID and lease token for this run.
3. Loop the steps in `references/coordinator-loop.md`: normalize the PR-review
   lane (Step 0), then claim and dispatch ready work with a lease.
