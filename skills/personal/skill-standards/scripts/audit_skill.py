...
KNOWN_DIRS = {"references", "scripts", "assets", "subskills", "agents"}
...
def check_links(pkg: Path, rep: Report, label: str) -> set[Path]:
    """Verify local markdown links resolve; return the set of linked files."""
    linked: set[Path] = set()
    for md in [pkg / "SKILL.md", *sorted((pkg / "references").glob("*.md"))]:
        if not md.exists():
            continue
...
def check_orphans(pkg: Path, linked: set[Path], rep: Report, label: str) -> None:
    for sub in ("references", "scripts"):
        for f in sorted((pkg / sub).glob("*")):
...
def check_pep723(pkg: Path, rep: Report, label: str) -> None:
    for py in sorted((pkg / "scripts").rglob("*.py")):
        if not PEP723_RE.search(py.read_text(encoding="utf-8")):
            rep.error(
...
def audit_package(pkg: Path, rep: Report, stale_days: int, label: str | None = None) -> None:
    ...
    linked = check_links(pkg, rep, label)
    check_orphans(pkg, linked, rep, label)
