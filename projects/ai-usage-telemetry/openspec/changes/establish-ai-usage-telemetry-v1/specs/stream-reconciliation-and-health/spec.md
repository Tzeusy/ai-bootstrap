...
MUST make one independently ordered source stream the indivisible cursor, parser-context, quarantine, freshness, reconciliation, coverage, and health unit; own generic regular-file stay-beneath traversal and source-stream generation from an injected runtime-owned `ValidatedSourceHandle`; accept only an adapter-owned filename predicate rather than importing an adapter manifest; keep canonical discovery order out of fact identity; and prevent one stream failure from quarantining sibling streams or source families. Runtime owns handle validation, adapters own filename predicates/manifests, and this capability alone owns discovery traversal/generation.
...
- **WHEN** generic discovery applies the two injected filename predicates to a validated handle and all resulting Claude and Codex streams contain complete supported records
...
MUST derive and persist each source cursor as exactly `(stream_generation,next_byte_offset,prefix_anchor,parser_context)`, where generic discovery canonically derives `stream_generation` from adapter schema ID, native stream identity, and platform generation evidence supplied through `ValidatedSourceHandle`; `next_byte_offset` is the unsigned start of the next record; `prefix_anchor` contains `algorithm="sha256"`, `schema_id`, profile-fixed `window_count`, `window_start_offset`, and the digest of RFC 8785 canonical arrays of registered kind ID, record-end offset, optional native identity and accounting fingerprint, and normalized context change; and `parser_context` is `{}` for Claude and for a newly created Codex byte-zero cursor before context, otherwise exact Codex session ID plus latest preceding turn timestamp, model, the domain-normalized source working-directory basename-or-null, and safe anchor, with no raw or content bytes and no repository-root inference.
...
- **AND** the stream-owned `LatchSet` proposes all seven closed dimensions with only `coverage` latched and every other dimension clear with initialization recovery evidence; the ledger persists that proposal in the same transaction, and the source component remains `runtime_state=coverage_unknown,state_code=coverage_unknown` until a complete reconciliation proposes clearing only coverage with recovery evidence and the ledger atomically commits the cursor, latch rows, checked `healthy` cache, null failure/state code, and `coverage_state=current`
...
MUST own one pure `LatchSet` interface with no SQLite or query dependency. Given the prior exact seven-dimension set and one dimension-owned latch/clear proposal, `LatchSet` MUST validate transition legality and evidence, preserve every sibling, return the next set, and derive the effective stream state/winning failure in exact order `storage_hold`, `quarantined`, `retention_gap`, `source_envelope_exceeded`, `reconciliation_overdue`, `trailing_deferred`, `coverage_unknown`, then `healthy`; each owning recovery clears only its row and reveals the next active lower row. The ledger MUST persist the proposed set and a checked derived cache, while query MUST project/validate that cache by invoking this interface; neither may reimplement transition, precedence, or recovery policy. The capability MUST expose one source-component row before stream existence and one row for every stream with its technical identity, enumerated current/recovery state, last successful scan, last accepted source time, complete durable cursor, held failure code/position, reconciliation evidence, and coverage/retention state; family/global summaries MUST remain degraded whenever any enabled member is unsupported, held, overdue, envelope-exceeded, retention-gapped, or coverage-unknown. Disabled source components MUST store `configured_state=disabled,runtime_state=disabled` with no stream/cursor/fact; enabling them re-enters source-profile activation and then `coverage_unknown`. Every stream MUST persist the exact `LatchSet` result as one `clear|latched` row for each closed dimension `storage|quarantine|retention|envelope|reconciliation|tail|coverage`, with dimension-matched evidence required by the ledger schema. Ledger unreadability composes above this source order as overall `unavailable`; readable ledger storage hold remains `storage_hold`. Overlap arrival permutations, restart, all clear orders, and duplicate-only success MUST produce the same result and preserve sibling evidence. Quota and sink states remain owned by their capabilities and are only composed upward by local query.
