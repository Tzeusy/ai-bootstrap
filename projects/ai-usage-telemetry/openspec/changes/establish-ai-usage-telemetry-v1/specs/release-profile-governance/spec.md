...
MUST encode the profile as RFC 8785 canonical JSON with exact top-level keys `schema="aiut.release-profile/v1"`, `profile_id`, `source_revision`, `dependency_lock_digest`, `adapter_profiles`, `identity_and_fingerprint_profile`, `parser_profile`, `reconciliation_profile`, `storage_profile`, `quota_freshness_profile`, `ledger_schema_digest`, `query_schema_digest`, `otlp_projection_profile`, `postgresql_projection_profile`, `runtime_profile`, `synthetic_evidence_inventory`, `capability_acceptance`, and `architecture_evidence`, with no extra keys, and compute its digest as SHA-256 over `UTF-8("aiut-release-profile-v1\n")` plus those canonical bytes; every nested member MUST include its schema ID, immutable values, evidence digests, and compatibility identifiers. Every amount/cumulative-counter `min_value` or `max_value` whose domain is the exact non-negative signed-64-bit integer range MUST use the identity-and-fingerprint domain's canonical decimal-string subtype rather than an RFC 8785 JSON number, with parsing and range validation delegated to that domain; profile composition MUST NOT own a second integer/string representation. `ledger_schema_digest` MUST cover every `STRICT` table, per-column storage-class constraints, component registrations, all seven independent per-stream latch rows and their closed failure/recovery evidence, fact-free quota states, sink registrations and digest domains, pending target metadata, health counters, and tagged null aggregate buckets; `query_schema_digest` MUST cover all exact view/JSON columns, latch-derived precedence, null rules, ordering, canonical encoding, fixed-nine-digit timestamp form, integer age arithmetic, and inspection-time exceptions; `otlp_projection_profile` MUST cover protocol, protobuf descriptor and encoder digests, deterministic/compression settings, encoded-size domain, and batch-vector schema; `postgresql_projection_profile` MUST cover exact request nullability, freshness checks, checked `bigint` Unix-nanosecond semantic time columns and equality, and deferred one-to-one commit constraints; and `runtime_profile` MUST cover the complete TOML grammar including globally distinct source namespaces and working-directory-basename alias semantics plus canonical sink-target algorithms.
...
- **WHEN** all required nested domain members, schema manifests, measured values, vectors, and evidence are present, including canonical amount/counter-bound strings for `9007199254740991`, `9007199254740992`, `9007199254740993`, and `9223372036854775807`
- **THEN** semantically identical JSON serializations produce the same canonical bytes and profile digest
- **AND** a numeric-token, non-canonical string, or the overflow value `9223372036854775808` for any such bound is invalid rather than rounded or redigested
...
MUST allow operator configuration only through the exact TOML grammar to select a profile-supported capability, choose distinct account and working-directory-basename presentation aliases and technical namespaces, require Claude and Codex `source_namespace` values to be globally distinct even when disabled, set polling interval where permitted, and narrow declared PostgreSQL or OTLP allowlists; configuration MUST NOT raise, replace, patch, or widen source-build membership, extraction paths, ledger admission, identity, fingerprint, arithmetic, freshness, parser, storage, reconciliation, vocabulary, encoding, or sink-budget values, and every effective sink alias/allowlist selection MUST enter its persisted projection-policy digest.
...
- **WHEN** TOML or environment attempts to increase a ceiling, add an extraction path or attribute, change arithmetic, replace a profile member, or give Claude and Codex equal source namespaces/native stream scope
...
MUST require every source identity and accounting rule and every bounded domain to carry version-pinned structural fixtures, positive and negative canonical vectors, replay, mutation, reset, malformed, privacy, and error cases, exactly-at and one-past boundary vectors, expected canonical bytes or semantic outputs, and applicable native amd64 and arm64 release results before activation. Every privacy/log/database/image/environment/network capture and parser-instrumentation lane MUST observe a distinct harmless positive-control canary and fail under deliberate test-only leak/decoder/network mutations. Closeout MUST also enumerate every one of the 249 named scenarios and every separately stated normative invariant and negative claim in current doctrine, RFC, design, and requirements, each mapped to a behavior-executing test and an independently implemented oracle; source scans and shared implementation/oracle code paths are insufficient.
...
- **WHEN** a rule is supported only by a local sample, lacks a negative or mutation vector, omits canonical-byte semantics, passes only under emulation, has an unobserved capture canary, fails to detect a deliberate mutation, or leaves any scenario/normative clause without an executable test and independent oracle
...
MUST require exact immutable values and units from the owning domain schemas for every parser count and byte ceiling, minimum memory, reconciliation deadline and envelope, storage charge, reserve and headroom, quota age and skew, diagnostic interval, OTLP UTF-8, tuple, series, SDK, and request ceiling, and any other RFC-declared numeric threshold, together with measurement method, minimum supported resources, evidence digest, and native boundary results; amount/cumulative-counter bounds in the non-negative signed-64-bit domain MUST retain their exact integer meaning while using the domain-owned canonical decimal-string representation in the RFC 8785 profile. No numeric value in this specification is a runtime default, and missing, unmeasured, inconsistently represented, overflowing, or unproved values keep the affected member inactive.
...
- **WHEN** every owning schema has exact accepted values and `N` and `N+1` evidence under the declared minimum resources on both native architectures, including amount/counter boundaries at `2^53-1`, `2^53`, `2^53+1`, signed-64-bit maximum, and overflow
