...
#### Scenario: Restart validates content-safe binding before secrets
- **WHEN** restart resolves the same canonical database, schema, and projection policy under the retained tuple
- **THEN** both persisted digests compare byte-equal before DSN credential read or connection and catch-up may resume
