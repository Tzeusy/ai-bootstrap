...
- **AND** one positive commit canary inserts an envelope with exactly one matching deferred child, while separate both-null, both-set, wrong-kind, orphan-envelope, orphan-child, and cross-kind-sequence transactions each fail at commit with no checkpoint movement

#### Scenario: Weakened schema blocks delivery
- **WHEN** any required identity component or constraint is missing, a usage and quota child can reuse one ledger sequence, an envelope can lack or mismatch its one child, an amount can outlive its event, utilization is unbounded, or checkpoint sequence can decrease
...
MUST treat a retry row as equal only when every shared envelope column and its exact fact-kind child linkage, every projected child column other than `projection_checkpoints.updated_at`, and every usage amount compare equal under PostgreSQL value equality, bytea identity and fingerprint compare byte-for-byte, canonical identity JSON text compares byte-for-byte, timestamps identify the same instant, numeric utilization compares exactly, amount rows match as a complete category-to-value set, and metadata equals `{}`; an unequal, orphaned, cross-kind, or multiply linked pre-existing row MUST block delivery and MUST never be overwritten.
...
- **WHEN** any existing identity row differs in fingerprint, normalized value, amount set, time, metadata, or technical envelope, or presents a both-null, both-set, wrong-kind, orphaned, multiply linked, or cross-kind-sequence state
...
- **WHEN** a migration would relax global cross-kind sequence uniqueness or one-to-one envelope linkage, relax another unique or foreign key, merge categories, reset a checkpoint, rewrite source time, or change equality semantics
