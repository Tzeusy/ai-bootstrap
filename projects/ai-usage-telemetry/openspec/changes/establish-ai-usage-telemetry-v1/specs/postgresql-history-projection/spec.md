...
MUST freeze `projection_schema_id="aiut-postgresql-history/v1"` and exactly these conceptual relations: `projected_facts(ledger_namespace text NOT NULL,ledger_epoch text NOT NULL,ledger_seq bigint NOT NULL CHECK(ledger_seq>0),fact_identity bytea NOT NULL,fact_kind text NOT NULL CHECK(fact_kind IN ('usage_event','quota_snapshot')),usage_fact_identity bytea NULL,quota_fact_identity bytea NULL,PRIMARY KEY(ledger_namespace,ledger_epoch,ledger_seq),UNIQUE(fact_identity),UNIQUE(ledger_namespace,ledger_epoch,ledger_seq,fact_identity,fact_kind),CHECK((fact_kind='usage_event' AND usage_fact_identity IS NOT NULL AND usage_fact_identity=fact_identity AND quota_fact_identity IS NULL) OR (fact_kind='quota_snapshot' AND quota_fact_identity IS NOT NULL AND quota_fact_identity=fact_identity AND usage_fact_identity IS NULL)))`; `usage_events(fact_identity bytea PRIMARY KEY,collector_namespace text NOT NULL,ledger_namespace text NOT NULL,ledger_epoch text NOT NULL,adapter_schema_id text NOT NULL,source_namespace text NOT NULL,fact_kind text NOT NULL CHECK(fact_kind='usage_event'),native_identity_json text NOT NULL,accounting_fingerprint bytea NOT NULL CHECK(octet_length(accounting_fingerprint)=32),ledger_seq bigint NOT NULL CHECK(ledger_seq>0),source_observed_unix_nano bigint NOT NULL CHECK(source_observed_unix_nano>=0),collected_unix_nano bigint NOT NULL CHECK(collected_unix_nano>=0),tool text NOT NULL,vendor text NOT NULL,model text NULL,project text NULL,native_request_identity_json text NULL,request_identity bytea NOT NULL,metadata jsonb NOT NULL CHECK(metadata='{}'::jsonb),UNIQUE(collector_namespace,ledger_namespace,adapter_schema_id,source_namespace,fact_kind,native_identity_json),FOREIGN KEY(ledger_namespace,ledger_epoch,ledger_seq,fact_identity,fact_kind) REFERENCES projected_facts(ledger_namespace,ledger_epoch,ledger_seq,fact_identity,fact_kind) DEFERRABLE INITIALLY DEFERRED)`; `usage_event_amounts(fact_identity bytea NOT NULL REFERENCES usage_events(fact_identity),category text NOT NULL,amount bigint NOT NULL CHECK(amount>=0),PRIMARY KEY(fact_identity,category))`; `quota_snapshots(fact_identity bytea PRIMARY KEY,collector_namespace text NOT NULL,ledger_namespace text NOT NULL,ledger_epoch text NOT NULL,adapter_schema_id text NOT NULL,source_namespace text NOT NULL,fact_kind text NOT NULL CHECK(fact_kind='quota_snapshot'),native_identity_json text NOT NULL,accounting_fingerprint bytea NOT NULL CHECK(octet_length(accounting_fingerprint)=32),ledger_seq bigint NOT NULL CHECK(ledger_seq>0),source_observed_unix_nano bigint NULL CHECK(source_observed_unix_nano IS NULL OR source_observed_unix_nano>=0),collected_unix_nano bigint NOT NULL CHECK(collected_unix_nano>=0),account_alias text NULL,vendor text NOT NULL,limit_name text NOT NULL,native_limit_identity text NOT NULL,native_window_identity text NOT NULL,native_scope_identity text NOT NULL,utilization numeric NOT NULL CHECK(utilization>=0 AND utilization<=1),window_minutes bigint NULL CHECK(window_minutes>=0),reset_unix_nano bigint NULL CHECK(reset_unix_nano IS NULL OR reset_unix_nano>=0),scope text NULL,freshness_state text NOT NULL CHECK(freshness_state IN ('fresh','stale','unknown')),freshness_evidence text NOT NULL CHECK(freshness_evidence IN ('record_timestamp','record_timestamp_and_window','record_timestamp_and_reset','record_timestamp_window_and_reset','missing_source_timestamp')),metadata jsonb NOT NULL CHECK(metadata='{}'::jsonb),UNIQUE(collector_namespace,ledger_namespace,adapter_schema_id,source_namespace,fact_kind,native_identity_json),FOREIGN KEY(ledger_namespace,ledger_epoch,ledger_seq,fact_identity,fact_kind) REFERENCES projected_facts(ledger_namespace,ledger_epoch,ledger_seq,fact_identity,fact_kind) DEFERRABLE INITIALLY DEFERRED)`; and `projection_checkpoints(sink_id text NOT NULL,destination_id text NOT NULL,projection_schema_id text NOT NULL CHECK(projection_schema_id='aiut-postgresql-history/v1'),ledger_epoch text NOT NULL,acknowledged_ledger_seq bigint NOT NULL CHECK(acknowledged_ledger_seq>=0),updated_at timestamptz NOT NULL,PRIMARY KEY(sink_id,destination_id,projection_schema_id,ledger_epoch))`. `projected_facts.usage_fact_identity` MUST have a `DEFERRABLE INITIALLY DEFERRED` foreign key to `usage_events(fact_identity)` and `projected_facts.quota_fact_identity` MUST have the same constraint to `quota_snapshots(fact_identity)`; each child MUST retain its `DEFERRABLE INITIALLY DEFERRED` composite foreign key back to the exact envelope tuple including identity and kind. The SQL-certain `IS NOT NULL` branches plus these deferred bidirectional constraints MUST make transaction commit accept exactly one same-identity, same-kind child and reject both pointers null, both pointers set, a selected pointer under the wrong kind, an orphaned envelope or child, and usage/quota reuse of `(ledger_namespace,ledger_epoch,ledger_seq)`; `projection_checkpoints` MUST have a database-enforced update guard that rejects any decrease of acknowledged sequence. The source, collection, and reset semantic-instant columns MUST be authoritative checked non-negative signed-64-bit UTC Unix nanoseconds; `projection_checkpoints.updated_at` MAY remain operational `timestamptz` because it is excluded from fact idempotency equality and never substitutes for source chronology.
...
- **AND** one positive commit canary inserts an envelope with exactly one matching deferred child and round-trips distinct adjacent Unix-nanosecond source, collection, and reset instants, while separate both-null, both-set, wrong-kind, orphan-envelope, orphan-child, cross-kind-sequence, negative-instant, overflow-instant, and timestamp-rounding transactions each fail with no checkpoint movement

#### Scenario: Weakened schema blocks delivery
- **WHEN** any required identity component or constraint is missing, a usage and quota child can reuse one ledger sequence, an envelope can lack or mismatch its one child, an amount can outlive its event, utilization is unbounded, a semantic instant uses `timestamptz` or an unchecked/negative bigint, adjacent nanoseconds collapse, or checkpoint sequence can decrease
- **THEN** schema validation fails and delivery is not enabled

#### Scenario: Shared sequence envelope is global across fact kinds
- **WHEN** one usage fact and one quota fact are inserted for the same ledger namespace and epoch
- **THEN** their `projected_facts` rows require distinct sequences and each envelope has exactly one child with the matching identity and `fact_kind`
- **AND** member-order permutations of both-null, both-set, wrong-kind, orphan/mismatched child, and cross-kind sequence reuse fail at transaction commit
...
MUST always project every technical identity, fingerprint, epoch, sequence, checkpoint, and checked Unix-nanosecond semantic instant field declared by the v1 schema, including non-null `request_identity` for every usage event; define the configurable descriptive-field allowlist as exactly `model|project|account_alias|native_request_identity_json`, default it to the empty set, and keep the v1 extension-metadata allowlist empty so `metadata` is exactly `{}`. If selected, `model`, `project`, and `account_alias` MUST preserve the normalized nullable value, while selected `native_request_identity_json` MUST preserve the normalized event's non-null canonical JSON; if unselected each corresponding destination column MUST be null. Configuration MUST NOT null or omit `request_identity`, substitute a `timestamptz` or rounded value for a semantic instant, or omit any other mandatory envelope field and MUST prohibit raw records, content, credentials, paths, or unregistered fields before serialization.
...
- **THEN** fact identity, source namespaces, fingerprint, ledger epoch and sequence, exact checked Unix-nanosecond source and collection instants, categories, and checkpoint identity remain present
...
- **THEN** one destination transaction exposes one globally ordered envelope and exactly one matching child for every sequence, exposes every usage amount, and advances its remote checkpoint to the batch end
...
MUST treat a retry row as equal only when every shared envelope column and its exact fact-kind child linkage, every projected child column other than `projection_checkpoints.updated_at`, and every usage amount compare equal under PostgreSQL value equality, bytea identity and fingerprint compare byte-for-byte, canonical identity JSON text compares byte-for-byte, each semantic source/collection/reset instant's checked `bigint` Unix-nanosecond value compares exactly, numeric utilization compares exactly, amount rows match as a complete category-to-value set, and metadata equals `{}`; `projection_checkpoints.updated_at` is operational and excluded from fact equality, while no `timestamptz`, rendered text, or rounded derivative may replace an authoritative semantic instant. An unequal, orphaned, cross-kind, multiply linked, or nanosecond-collapsed pre-existing row MUST block delivery and MUST never be overwritten.
...
- **WHEN** PostgreSQL committed an equal batch but local acknowledgement was lost, including rows whose semantic instants differ from another retained row by exactly one nanosecond
- **THEN** retry recognizes only bit-exact Unix-nanosecond equality, leaves equal rows unchanged, and may advance the remote and then local checkpoint monotonically

#### Scenario: Conflicting idempotency row blocks
- **WHEN** any existing identity row differs in fingerprint, normalized value, amount set, exact Unix-nanosecond instant, metadata, or technical envelope, collapses adjacent nanoseconds, or presents a both-null, both-set, wrong-kind, orphaned, multiply linked, or cross-kind-sequence state
...
MUST preserve each exact source-observed event or snapshot instant as checked non-negative signed-64-bit UTC Unix nanoseconds independently of the separately retained collection instant, retry, batch, destination commit, and delivery time; preserve quota reset time by the same representation; keep null only where quota source or reset time is contractually unavailable; and never substitute a later, rounded, or `timestamptz` value for source chronology.
...
- **THEN** its source and collection instants remain distinct checked `bigint` Unix-nanosecond destination columns with their original meanings, including when their values or neighboring facts differ by exactly one nanosecond
...
- **THEN** its destination row keeps the bit-exact source Unix-nanosecond value and does not substitute delivery, collection, rendered text, or a rounded timestamp

### Requirement: [TARGET-STATE] Lossless PostgreSQL Schema Evolution
MUST use explicit ordered migrations with immutable artifact digests and executable pre/post vectors, apply each migration transactionally, preserve every projected row and value, global `(ledger_namespace,ledger_epoch,ledger_seq)` uniqueness across fact kinds, exactly one matching child per shared envelope, full identity uniqueness, amount foreign-key completeness, bit-exact checked Unix-nanosecond source/collection/reset instants, category separation, exact idempotency comparisons, and monotonic checkpoints, and keep tuning, indexes, and tool choice from weakening those constraints. A prior-schema migration MUST populate envelopes from the union of usage and quota rows without renumbering or rounding any semantic instant and MUST fail atomically if a cross-kind sequence collision, orphan, invalid instant, or lossless nanosecond conversion failure prevents the exact target shape.
...
- **THEN** post-migration logical rows, uniqueness, equality behavior, bit-exact semantic Unix-nanosecond instants including adjacent values, categories, and checkpoints match the declared oracle

#### Scenario: Constraint-weakening migration fails
- **WHEN** a migration would relax global cross-kind sequence uniqueness or one-to-one envelope linkage, relax another unique or foreign key, merge categories, reset a checkpoint, rewrite or round a semantic instant, collapse adjacent nanoseconds, or change equality semantics
- **THEN** migration fails transactionally and projection remains blocked

#### Scenario: Prior rows migrate without sequence renumbering
- **WHEN** a supported prior destination contains usage and quota rows with distinct ledger sequences
- **THEN** migration builds one matching envelope per retained row, preserves every sequence and idempotency value, and passes the target constraints
- **AND** separate prior fixtures with both-null, both-set, wrong-kind, orphaned, or cross-kind-sequence states each fail all-old-or-all-new without changing any row or checkpoint
