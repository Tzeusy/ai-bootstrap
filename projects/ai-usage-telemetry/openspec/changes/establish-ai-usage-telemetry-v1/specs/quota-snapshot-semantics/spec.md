...
MUST define a source-independent `QuotaSnapshot` interface and admit a value only with canonical fact identity, SHA-256 fingerprint, nullable registry-derived source-observed time, collected time, nullable configured account alias, vendor, limit name, native limit, window, and scope identities, canonical utilization decimal in inclusive `0.0..1.0`, optional window minutes, reset time, and scope, recorded freshness state, one exact evidence enum, and empty v1 metadata. Every non-null source, collection, and reset instant MUST use the sole domain-owned canonical-instant parser/renderer: explicit-offset RFC 3339 with year `0001..9999`, no leap second, checked conversion to a non-negative signed-64-bit UTC Unix-nanosecond integer, and only `YYYY-MM-DDTHH:MM:SS.nnnnnnnnnZ` rendering; absent account alias MUST remain null without a default, is presentation-only, and is never a quota subject or fact-identity member. The domain-owned fingerprint function MUST compute SHA-256 over `UTF-8("aiut-accounting-fingerprint-v1\n")` plus RFC 8785 canonical UTF-8 JSON of exactly `{"adapter_schema_id":string,"fact_kind":"quota_snapshot","native_identity":json,"source_observed_at":RFC3339-UTC-string-or-null,"quota":{"vendor":string,"native_limit_identity":string,"limit_name":string,"utilization":canonical-decimal-string,"native_window_identity":string,"window_minutes":non-negative-integer-or-null,"reset_at":RFC3339-UTC-string-or-null,"native_scope_identity":string,"scope":string-or-null,"freshness_evidence":enum}}`, excluding collected time, account alias, current freshness state, paths, metadata, and sinks. Adapters supply registered quota fields/evidence and invoke this interface; the quota domain MUST NOT import an adapter module.
...
MUST require each accepted source-and-limit member to provide immutable unsigned `maximum_age_seconds` and `allowed_future_skew_seconds` with measurement digest and native evidence, and MUST restrict `freshness_evidence` to exactly `record_timestamp`, `record_timestamp_and_window`, `record_timestamp_and_reset`, `record_timestamp_window_and_reset`, or `missing_source_timestamp`. The quota domain's one checked age function is the sole implementation owner of every multiplication, subtraction, clamp, and final floor on normalized integer nanoseconds. A known future source time is admitted only through the inclusive skew bound and maps to `age_ns=0,age_seconds=0,fresh`; beyond-skew or overflow is malformed. Otherwise `age_ns=collected_unix_nano-source_unix_nano`, `age_seconds=floor(age_ns/1_000_000_000)`, freshness is `fresh` through inclusive `age_ns=maximum_age_seconds*1_000_000_000`, and the first nanosecond later is `stale`; absent source time is `unknown`, while collected time never substitutes for source time or enters the fingerprint. Query and OTLP consumers MUST invoke this function and test it against independently implemented oracles rather than duplicate its arithmetic.
...
SHALL make the read-only quota view emit one `component_state` row for each registered quota component and one `snapshot` row per retained fact, invoke the domain-owned age function to recompute current snapshot `fresh` or `stale` and `age_seconds=floor(max(0,inspection_unix_nano-source_unix_nano)/1_000_000_000)` against inspection time using the immutable source-and-limit threshold while preserving recorded evidence, source time, collection time, and observation state, and keep `unavailable`, `absent`, `null`, `state_only`, stale, and unknown queryable under their exact fact/null rules without making a live-vendor-quota claim. The view/query implementation MUST NOT reimplement the arithmetic.
