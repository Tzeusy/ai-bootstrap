...
- **WHEN** Claude and Codex fixtures include root/null, nested, and non-repository working directories and configure distinct aliases only for the valid nested and non-repository basenames
- **THEN** the basename mappings validate independently, root/null remains unaliased null, canonical project identity and null-tagged aggregates remain unchanged, and only the affected sink policy digest and presentation change

#### Scenario: Secret or namespace mutation fails safely
- **WHEN** TOML contains a DSN, token, raw record, unknown key, changed technical namespace, or equal Claude/Codex source namespaces including a same-native-stream fixture
- **THEN** startup fails before component registration, fixed-clock health bytes, scanning, credential access, export, or logging the value
...
- **AND** every packet/environment capture observes its harmless positive-control canary before a clean result counts, while a deliberate test-only unexpected network event makes the harness fail
