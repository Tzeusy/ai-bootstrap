...
MUST require every active adapter member to provide unsigned inclusive `max_record_bytes` measured from record start excluding the delimiter, `max_depth` with the root at one, `max_keys_per_record`, `max_encoded_key_bytes`, `max_projected_occurrences`, `max_application_memory_bytes`, and `max_structural_steps`, plus for every projected path inclusive `max_encoded_bytes`, `max_decoded_utf8_bytes`, `max_multiplicity`, integer `min_value` and `max_value` or decimal `min_value`, `max_value`, `max_precision`, and `max_scale`; all values, the minimum supported memory in bytes, counting algorithms, measurement evidence digests, and architecture results are immutable profile inputs. An integer `min_value` or `max_value` for a non-negative signed-64-bit amount/cumulative-counter path MUST use the source-independent domain's canonical non-negative signed-64-bit decimal-string subtype in the RFC 8785 release-profile document rather than a JSON number, while the parser and domain value remain an exact checked integer. Before traversal the exact phase order MUST be complete profile validation, receipt of an injected runtime-owned `ValidatedSourceHandle`, and receipt of an injected ledger/storage-owned `AdmissionDecision=permitted`. Parser and adapter modules MUST consume those interfaces, remain testable with fake handles and fake permitted/denied decisions, and MUST NOT implement or import concrete runtime/storage preflight. After traversal begins, compound failures MUST use the JSON-member-order and chunk-order independent precedence `record_limit` for any measured/checked parser-resource or representation overflow, `schema_inconsistent` for invalid UTF-8 or JSON structure, `unknown_kind` for a valid unregistered discriminator, `recognized_malformed` for a registered kind's missing/wrong projected value or multiplicity or a safely represented value outside its source-semantic domain, then `unregistered_category`; only parser success may reach later `identity_collision`. In particular, a safely represented Codex `used_percent` outside source-semantic inclusive `0..100` is `recognized_malformed`, while only overflow of its active measured parser/representation bound is `record_limit`. A missing required profile member, handle, permitted decision, bound, counting rule, or evidence result MUST keep or hold the source before traversal. Every exact `N` is admitted and no library or operator default is substituted.
...
- **WHEN** a record and its projected values equal every applicable active-profile ceiling without exceeding the combined memory and work bounds, including amount/counter bounds at `9007199254740991`, `9007199254740992`, `9007199254740993`, and `9223372036854775807`
- **THEN** limit validation permits semantic classification
- **AND** measured memory remains within the profile's minimum-resource claim and every such bound has distinct canonical decimal-string bytes

#### Scenario: One-past unterminated record fails immediately
- **WHEN** a record crosses its raw-byte cap without a JSONL delimiter, any other measured bound reaches `N+1`, or an amount/counter reaches the overflow value `9223372036854775808`
...
MUST define `claude-code/session-jsonl@1` with canonical target `/sources/claude/sessions` and adapter-owned filename predicate `*.jsonl`; generic stream discovery alone owns regular-file, non-symlink, stay-beneath traversal and generation. The adapter MUST classify `/type="assistant"` as usage-bearing; project bounded strings at `/type`, `/sessionId`, `/requestId`, `/timestamp`, `/cwd`, `/message/id`, and `/message/model`; project exact non-negative signed-64-bit integers at `/message/usage/input_tokens`, `/message/usage/cache_creation_input_tokens`, `/message/usage/cache_read_input_tokens`, and `/message/usage/output_tokens`; classify `/message/content` as content-bearing; and require each accepted source-build member to digest-cover its complete finite non-wildcard set of non-`assistant` irrelevant `/type` values before activation.
...
MUST supply Claude native/logical request identity evidence `(sessionId,requestId)` and mandatory `message.id` consistency evidence to the source-independent domain interfaces; map source amount fields to the exact registered categories; preserve absent optional amounts as absent; and invoke the domain-owned canonical instant, cwd-basename, identity, request, category, fingerprint, and canonical non-negative signed-64-bit decimal-string functions rather than reimplementing them. The adapter MUST keep parsed amounts as checked integers and leave their RFC 8785 string representation to the domain; use the record's own timestamp/model/cwd fields; never perform repository discovery or ancestor selection; keep every project alias outside the normalized event, fact/request identity, accounting fingerprint, and aggregate keys; expose aliases only through presentation and digest-bound sink policy; and quarantine same-identity changes in time, message identity, model, or amount.
...
MUST define `codex/rollout-jsonl@1` with canonical target `/sources/codex/sessions` and adapter-owned filename predicate `rollout-*.jsonl`; generic stream discovery alone owns regular-file, non-symlink, stay-beneath traversal and generation. The adapter MUST admit exact top-level discriminator values `session_meta`, `turn_context`, and `event_msg`; classify only `/type="event_msg"` plus `/payload/type="token_count"` as fact-bearing; project strings at `/timestamp`, `/type`, `/payload/type`, `/payload/id` for `session_meta`, and `/payload/model` and `/payload/cwd` for `turn_context`; project exact non-negative signed-64-bit integers named `input_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, `output_tokens`, `reasoning_output_tokens`, and `total_tokens` beneath both `/payload/info/total_token_usage` and `/payload/info/last_token_usage`; supply those checked integers to the source-independent domain, which alone renders every cumulative-counter leaf in Codex native fact/request identities and every emitted fingerprint amount through its canonical decimal-string subtype; register `/payload/rate_limits` itself as an optional `object|null` presence path, project its `/limit_id` as a bounded string and each registered `primary` or `secondary` window's `used_percent` as an exact decimal in `0..100` and optional `window_minutes` and `resets_at` as non-negative integers; classify a missing member as `absent`, explicit null as `null`, an admitted non-null object containing only exact profile-allowed identity/context members and no registered primary/secondary window object as `state_only`, and one or more complete registered windows as `observed`; require any present registered primary/secondary window member to have its admitted object shape and required correctly typed utilization in inclusive `0..100`, otherwise classify the whole record `recognized_malformed` and roll back every same-record usage, quota, component-state, and cursor effect; and require each source-build member to digest-cover its complete finite non-wildcard irrelevant envelope and payload-value table before activation.
...
- **WHEN** a complete token-count record matches the exact envelope, context, usage, and rate-limit types of an accepted build member, including cumulative counters at `9007199254740991`, `9007199254740992`, `9007199254740993`, and `9223372036854775807`
...
- **AND** each accepted counter remains an exact integer internally and has distinct canonical decimal-string identity/fingerprint bytes
...
- **WHEN** otherwise-equal supported records omit `rate_limits`, set it to JSON null, provide only exact profile-allowed identity/context with no registered window, provide a complete primary window, or provide a registered primary/secondary window with missing, mistyped, or out-of-range utilization
- **THEN** the first four quota candidate outcomes are respectively `absent`, `null`, `state_only`, and `observed` without treating any as zero
- **AND** each malformed present window makes the whole record `recognized_malformed`, rolls back all same-record effects, and is never admitted as `state_only` or `record_limit`
...
- **WHEN** every cumulative component advances according to an accepted rule and the computed delta agrees with the declared last-usage relation across `2^53-1`, `2^53`, `2^53+1`, and signed-64-bit-maximum boundary vectors
- **THEN** the adapter emits the exact non-overlapping delta and one logical-request contribution
- **AND** an unchanged landmark emits no usage fact even when quota facts are present, and each distinct boundary retains distinct canonical native-identity, request-identity, and fingerprint bytes
