...
- **AND** source component columns MUST come from `component_registrations`; each stream's `state`, `failure_code`, and `failure_offset` MUST project the ledger's checked `source_streams` cache, whose agreement with `stream_health_latches` is validated by invoking the stream-owned `LatchSet` rather than reimplementing precedence, and all other stream columns come from `source_streams`; sink registration/checkpoint columns come from `component_registrations`, `sink_registrations`, and `sink_checkpoints`, and ledger identity/counter columns from `ledger_state` plus active `release_profile_state`; each synthetic `row_kind`, summary, admission, backlog, or renamed lease field MUST use only the owner-produced values or derivations closed below
...
MUST restrict stream rows' `source_health.state` to `healthy`, `trailing_deferred`, `quarantined`, `storage_hold`, `reconciliation_overdue`, `source_envelope_exceeded`, `retention_gap`, or `coverage_unknown`; restrict source-component rows to `disabled`, `unsupported_profile`, `unsupported_accounting_profile`, `coverage_unknown`, `healthy`, or `degraded`; project each stream state and winning failure from the ledger's checked cache, validate that cache against the seven persistent latch rows by invoking the stream-owned `LatchSet`, expose the declared component/per-stream fields under their exact null rules, derive `family_state` and `global_state` as `healthy`, `degraded`, or `disabled`, and prohibit either summary from masking any enabled unsupported, held, overdue, envelope-exceeded, or coverage-degraded component. Query code MUST NOT implement its own latch transition, precedence, or recovery function; its tests MUST use an independently implemented expected-row oracle.
...
MUST restrict ledger migration state to `current`, `pending`, or `failed`; integrity state to `verified`, `unverified`, or `failed`; availability state to `available`, `storage_hold`, or `ledger_unavailable`; and admission state to `admissible`, `insufficient_capacity`, or `unknown`; expose namespace, epoch, schema, profile ID and digest, current capacity/reserve evidence and `AdmissionDecision` supplied by the ledger/storage provider, last sequence and transaction, and the durable accepted-fact, duplicate-candidate, and distinct-hold-episode counts owned by the ledger transaction contract; and project a current read-only hold or unavailable result even when persisted health is stale. Query MUST NOT reimplement storage charge/admission arithmetic.
...
- **AND** `available_bytes`, `admission_charge_bytes`, `reserve_bytes`, and `admission_state` MUST project the ledger/storage provider's current read-only evidence and exact `AdmissionDecision` under REQ-durable-local-ledger-008; query validates their closed mapping without recomputing the capacity inequality, and `persisted_health_stale` MUST be `1` only when direct inspection evidence is newer than or cannot be written to persisted state, otherwise `0`
...
Normative continuation: for this exact contract, query MUST invoke the
domain-owned canonical-instant renderer for every RFC 3339 timestamp and the
quota-domain age function for every `age_seconds` value
`floor(max(0,inspection_unix_nano-source_unix_nano)/1_000_000_000)`. Tolerated
future timestamps therefore have age zero; an out-of-range instant, leap
second, or arithmetic overflow fails closed rather than using a query-local,
host-datetime, or floating-point implementation. Query tests use independently
implemented byte/age oracles, not the production functions as expected values.
